...
function execute (opts) {
  ...
  return transactions.calculateInstallmentsAmount(payload, { amount: 1, interest_rate: 100 })
    ...
    .catch({ name: 'ApiError' }, () => reject(new Error('You must supply a valid encryption key')))
    .catch(({ name }) => console.warn(`Warning: Could not verify key. Pagar.me may be offline ${name}`))
    .then(merge(payload))
    .then(requestOpts => ({
